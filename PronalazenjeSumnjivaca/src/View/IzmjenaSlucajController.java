...
public class IzmjenaSlucajController implements Initializable, ControlledScreen, DialogManager {
    ...
    ObservableList<String> observableOsumnjiceni;
    ObservableList<String> observableSvjedoci;
    ObservableList<String> observableDokazi;
    ObservableList<String> observablePolicajci;
    ObservableList<String> observableDogadaji;
    ObservableList<String> observableFotografije;
    ...
    public void postaviTrenutnePodatke(Slucaj slucaj) {
        ...
        observableOsumnjiceni = FXCollections.observableList(lista);
        if (!observableOsumnjiceni.get(0).equals("")) {
        ...
        observableSvjedoci = FXCollections.observableList(lista);
        if (!observableSvjedoci.get(0).equals("")) {
        ...
        observableDokazi = FXCollections.observableList(lista);
        if (!observableDokazi.get(0).equals("")) {
        ...
        observablePolicajci = FXCollections.observableList(lista);
        if (!observablePolicajci.get(0).equals("")) {
        ...
        observableDogadaji = FXCollections.observableList(lista);
        if (!observableDogadaji.get(0).equals("")) {
            popisDogadaja.setItems(observableDogadaji);
        }
        
        lista.clear();
        lista.addAll(slucaj.getFotografijeSlučaja());
        observableFotografije = FXCollections.observableList(lista);
        if (!observableFotografije.get(0).equals("")) {
    ...
    @FXML private void spremiIzmjene(ActionEvent event) {
        slucaj.setNazivSlucaja(naziv.getText());
        slucaj.setOpis(opis.getText());
        
        String[] zapis = glavniOsumnjiceni.getText().split(" ");
        if (!slucaj.getGlavniOsumnjiceni().getOib().equals(Long.parseLong(zapis[2].trim()))) {
            Osumnjiceni novi = new Osumnjiceni();
            novi.setOib(Long.parseLong(zapis[2].trim()));
            slucaj.setGlavniOsumnjiceni(novi);
        }
        ...
        delegate.spremiIzmjeneSlucaja(slucaj);
    }
